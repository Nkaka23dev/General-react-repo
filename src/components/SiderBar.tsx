    ...
    return (
        <div className='border-r-2 border-gray-300  h-screen'>
            <div className='flex justify-between my-5 items-baseline '>
                ...
                <div className='pr-3'>
                    <button onClick={() => onAddNote()} className='text-sky-500 font-bold text-md hover:underline'>{valueToEdit.title || valueToEdit.body ? "Edit" : "add"}</button>
                </div>
            </div>
            {/* content  */}
            {notes.map((note: any, idx: number) => {
                return (
                    <div onClick={() => setValueToEdit(note)} key={idx} className={`flex pb-6 justify-between my-5 ${valueToEdit?.id === note?.id ? 'shadow-2xl' : ''} items-baseline cursor-pointer hover:shadow-lg`}>
                        <div className=''>
